...
- 新增 `simulation ls/show/positions/orders/returns/sync`：列表、持仓与模拟仓下单历史、历史累计收益、当日日内收益，以及本地 JSON 快照。
- 逐页读取列表和历史收益，逐日读取持仓与下单记录；检测截断、重复页和异常响应，失败不覆盖旧快照。
- 明确北京时间日期、实际持仓日期、累计收益百分数和最新统计小数比例，避免非交易日误标。
- 新增模拟盘测试 22 项；合并最新研究平台功能后，PR 测试总数为 292 项，验证结果见 PR。工作区另有 1 项原有输出编码测试，不纳入本 PR。
- 真实只读验证：7 个模拟盘最近两天快照同步成功；另验证单个模拟仓的一周持仓与下单记录、完整历史收益和非交易日空日内收益。
- README、DESIGN、仓库 jqcli skill 的入口、工作流、故障排查和界面提示已同步更新。

以下为此前的历史开发记录，账号状态、测试数及样例 ID 不代表当前状态。
